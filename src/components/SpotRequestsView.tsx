...
import { decideOperationalRequest } from '../lib/operations';
import { BicycleSpot, RegisteredBicycle } from '../types';
import { isAllocatedToBike } from '../utils/bicycleIdentity';
...
export function SpotRequestsView({ condominiumId, spots, bikes, canManage, initialFocus = 'all', onAllocate, onBackToTasks, refreshKey = 0 }: { condominiumId: string | null; spots: BicycleSpot[]; bikes: RegisteredBicycle[]; canManage: boolean; initialFocus?: 'all' | 'queue' | 'irregularities'; onAllocate: (request: SpotRequest, spot: BicycleSpot) => void; onBackToTasks?: () => void; refreshKey?: number }) {
  ...
  const [viewFilter, setViewFilter] = useState<'all' | 'queue' | 'irregularities'>(initialFocus);
  ...
  useEffect(() => { void load(); }, [condominiumId, refreshKey]);
  useEffect(() => setViewFilter(initialFocus), [initialFocus]);

  const freeSpots = useMemo(() => spots.filter((spot) => !spot.currentAllocation), [spots]);
  const selectableBikes = useMemo(() => recordType === 'vaga'
    ? bikes.filter((bike) => !bike.spotId && !bike.spotNumber && !spots.some((spot) => isAllocatedToBike(spot, bike)))
    : bikes, [bikes, recordType, spots]);
  const openRecords = useMemo(() => requests.filter((request) => request.status === 'pending' || request.status === 'waiting_list'), [requests]);
  const visibleOpenRecords = useMemo(() => openRecords.filter((request) => viewFilter === 'all'
    || (viewFilter === 'queue' && request.request_type === 'vaga')
    || (viewFilter === 'irregularities' && (request.request_type === 'ocorrencia' || request.request_type === 'regularizacao'))), [openRecords, viewFilter]);
  ...
  const reject = async (request: SpotRequest) => {
    if (!condominiumId || !window.confirm('Recusar este pedido de vaga?')) return;
    setDecidingRequestId(request.id);
    try {
      await decideOperationalRequest({ condominiumId, requestId: request.id, action: 'reject' });
      await load();
    } catch (error) {
      setMessage(typeof error === 'object' && error && 'message' in error ? String(error.message) : 'Não foi possível atualizar o registro.');
  ...
  const complete = async (request: SpotRequest) => {
    ...
    setDecidingRequestId(request.id);
    try {
      await decideOperationalRequest({ condominiumId, requestId: request.id, action: 'complete' });
      await load();
    } catch (error) {
      setMessage(typeof error === 'object' && error && 'message' in error ? String(error.message) : 'Não foi possível concluir o registro.');
    } finally { setDecidingRequestId(null); }
  };
  ...
  const convertToOccurrence = async (request: SpotRequest) => {
    if (!condominiumId || !window.confirm('Converter este registro em ocorrência? A atribuição de vaga será removida e a situação seguirá para Irregularidades.')) return;
    setDecidingRequestId(request.id);
    try {
      await decideOperationalRequest({ condominiumId, requestId: request.id, action: 'convert_to_occurrence' });
      setActiveRequestId(null);
      setMessage('Registro convertido em ocorrência e enviado para Irregularidades.');
      await load();
    } catch (error) {
      setMessage(typeof error === 'object' && error && 'message' in error ? String(error.message) : 'Não foi possível converter o registro em ocorrência.');
    } finally { setDecidingRequestId(null); }
  };
  ...
  return <div className="mx-auto w-full max-w-7xl p-4 sm:p-6">{onBackToTasks && <button type="button" onClick={onBackToTasks} className="mb-4 inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 transition hover:border-orange-300 hover:bg-orange-50"><ArrowLeft className="h-3.5 w-3.5 text-[#e87c0b]" />Voltar para tarefas</button>}<div className="grid gap-6 lg:grid-cols-[360px_1fr]">
    ...
    <section className="brand-module"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#e87c0b]">Central operacional</p><h1 className="mt-1 text-lg font-black">Registros e solicitações</h1><p className="mt-1 text-xs text-slate-500">Atribua uma vaga somente para pedidos de vaga. Ocorrências e vistorias têm seu próprio fluxo.</p></div><button type="button" onClick={() => void load()} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold hover:bg-slate-50">Atualizar</button></div><div className="mt-4 grid gap-2 sm:grid-cols-3"><Summary label="Em aberto" value={openRecords.length} tone="amber" /><Summary label="Vagas livres" value={freeSpots.length} tone="emerald" /><Summary label="Concluídos" value={completedRecords.length} tone="slate" /></div><div className="mt-4 flex flex-wrap gap-2" aria-label="Filtrar registros"><RequestFilter active={viewFilter === 'all'} onClick={() => setViewFilter('all')} label="Todos" count={openRecords.length} /><RequestFilter active={viewFilter === 'queue'} onClick={() => setViewFilter('queue')} label="Fila de vagas" count={openRecords.filter((request) => request.request_type === 'vaga').length} /><RequestFilter active={viewFilter === 'irregularities'} onClick={() => setViewFilter('irregularities')} label="Irregularidades" count={openRecords.filter((request) => request.request_type === 'ocorrencia' || request.request_type === 'regularizacao').length} /></div>{loading ? <p className="py-12 text-center text-sm text-slate-500">Carregando registros…</p> : <div className="mt-5 space-y-3">{visibleOpenRecords.map((request) => <RecordCard key={request.id} request={request} active={activeRequestId === request.id} busy={decidingRequestId === request.id} canManage={canManage} freeSpots={freeSpots} selectedSpotId={selectedSpotIds[request.id] || ''} onToggle={() => setActiveRequestId((current) => current === request.id ? null : request.id)} onSpotChange={(spotId) => setSelectedSpotIds((current) => ({ ...current, [request.id]: spotId }))} onApprove={() => approveAllocation(request)} onReject={() => void reject(request)} onComplete={() => void complete(request)} onConvertToOccurrence={() => void convertToOccurrence(request)} />)}{visibleOpenRecords.length === 0 && <div className="rounded-2xl border border-dashed border-emerald-200 bg-emerald-50/50 py-12 text-center text-sm text-emerald-800"><Check className="mx-auto mb-2 h-6 w-6" />Nenhum registro neste assunto aguarda tratamento.</div>}<button type="button" onClick={() => setShowHistory((current) => !current)} className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-xs font-bold text-slate-600">Histórico de decisões <span className="inline-flex items-center gap-2">{completedRecords.length}<ChevronDown className={`h-4 w-4 transition-transform ${showHistory ? 'rotate-180' : ''}`} /></span></button>{showHistory && <div className="space-y-2">{completedRecords.slice(0, 8).map((request) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2"><span className="text-xs font-bold text-[#0d1733]">{recordCopy[request.request_type || 'vaga'].label} · {request.resident_name} <small className="font-mono text-slate-500">Apto {request.apartment}</small></span><Status status={request.status} /></div>)}{completedRecords.length === 0 && <p className="py-3 text-center text-xs text-slate-500">Ainda não há decisões registradas.</p>}</div>}</div>}</section>
  </div></div>;
}
...
function RecordCard({ request, active, busy, canManage, freeSpots, selectedSpotId, onToggle, onSpotChange, onApprove, onReject, onComplete, onConvertToOccurrence }: { request: SpotRequest; active: boolean; busy: boolean; canManage: boolean; freeSpots: BicycleSpot[]; selectedSpotId: string; onToggle: () => void; onSpotChange: (spotId: string) => void; onApprove: () => void; onReject: () => void; onComplete: () => void; onConvertToOccurrence: () => void }) {
  ...
  if (busy) return <article className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold text-slate-600" aria-busy="true">Confirmando a decisão com o condomínio…</article>;
...
function RequestFilter({ active, onClick, label, count }: { active: boolean; onClick: () => void; label: string; count: number }) { return <button type="button" aria-pressed={active} onClick={onClick} className={`rounded-lg border px-3 py-2 text-xs font-black transition ${active ? 'border-[#0d1733] bg-[#0d1733] text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-orange-300'}`}>{label} <span className={`ml-1 rounded px-1.5 py-0.5 text-[10px] ${active ? 'bg-white/15 text-white' : 'bg-slate-100 text-slate-500'}`}>{count}</span></button>; }
