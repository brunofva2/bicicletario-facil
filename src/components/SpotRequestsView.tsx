...
import { ArrowLeft, Check, ChevronDown, ClipboardCheck, ListPlus, MapPin, X } from 'lucide-react';
...
function RecordCard({ request, active, canManage, freeSpots, selectedSpotId, onToggle, onSpotChange, onApprove, onReject, onComplete }: { request: SpotRequest; active: boolean; canManage: boolean; freeSpots: BicycleSpot[]; selectedSpotId: string; onToggle: () => void; onSpotChange: (spotId: string) => void; onApprove: () => void; onReject: () => void; onComplete: () => void }) {
  ...
  return <article className="rounded-2xl border border-orange-100 bg-[#fffaf2] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-white px-2 py-1 text-[9px] font-black uppercase tracking-wide text-[#b96500]">{copy.label}</span>{!isAllocation && <span className={`rounded-md px-2 py-1 text-[9px] font-black uppercase ${severityStyle}`}>{request.severity === 'urgent' ? 'Urgente' : request.severity === 'attention' ? 'Atenção' : 'Normal'}</span>}</div><p className="mt-2 font-bold">{request.resident_name} <span className="font-mono text-xs text-slate-500">· Apto {request.apartment}{request.block ? ` / ${request.block}` : ''}</span></p><p className="mt-1 text-xs text-slate-500">{request.bicycle_description || 'Bicicleta não detalhada'}{request.resident_phone ? ` · ${request.resident_phone}` : ''}</p>{request.notes && <p className="mt-2 inline-flex items-center gap-1 rounded-lg bg-white px-2 py-1 text-[10px] font-semibold text-slate-600">{isAllocation && <MapPin className="h-3 w-3 text-[#e87c0b]" />}{isAllocation ? `Preferência: ${request.notes}` : request.notes}</p>}</div><Status status={request.status} /></div>{canManage && <button type="button" onClick={onToggle} className="mt-3 text-[10px] font-black text-[#b96500]">{active ? 'Fechar tratamento' : copy.action}</button>}{canManage && active && <div className="request-allocation-panel mt-4 border-t border-orange-100 pt-3">{isAllocation ? <><p className="text-[10px] font-bold text-slate-600">Escolha a vaga livre que será atribuída. A preferência é apenas uma referência; a decisão é da gestão.</p><div className="mt-3 flex flex-wrap gap-2"><select aria-label={`Vaga para ${request.resident_name}`} value={selectedSpotId} onChange={(event) => onSpotChange(event.target.value)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs"><option value="">Escolha uma vaga livre</option>{freeSpots.map((spot) => <option key={spot.id} value={spot.id}>{spot.spotNumber} · {spot.sector}</option>)}</select><button type="button" disabled={freeSpots.length === 0} onClick={onApprove} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50"><Check className="h-3.5 w-3.5" />Aprovar e atribuir</button><button type="button" onClick={onReject} className="inline-flex items-center gap-1 rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-700 hover:bg-rose-50"><X className="h-3.5 w-3.5" />Recusar</button></div>{freeSpots.length === 0 && <p className="mt-2 text-[10px] font-bold text-rose-600">Não há vagas livres para atribuição neste momento.</p>}</> : <><p className="text-[10px] font-bold text-slate-600">Este registro não altera a ocupação de vagas. Conclua quando a ação descrita tiver sido realizada.</p><button type="button" onClick={onComplete} className="mt-3 inline-flex items-center gap-1 rounded-lg bg-[#0d1733] px-3 py-2 text-xs font-bold text-white"><ClipboardCheck className="h-3.5 w-3.5" />Concluir registro</button></>}</div>}</article>;
}
